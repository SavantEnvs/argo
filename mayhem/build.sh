#!/usr/bin/env bash
#
# mayhem/build.sh — build argo-workflows' PROGRESS field parser
# (pkg/apis/workflow/v1alpha1/progress.go:ParseProgress) as a sanitized libFuzzer
...
#
# HARNESS STAGING (netnew §6 Go / port-go): the real argo-workflows module pins
# `go 1.26.5` and its pkg/apis/workflow/v1alpha1 package drags the FULL Kubernetes
# api-machinery closure through go.mod. ParseProgress() and its helpers, though,
# need nothing but stdlib (fmt + strconv + strings). So we copy JUST progress.go
# into a fresh STANDALONE Go module at _mayhem_harness/progress and build there.
# The harness module's only non-stdlib dep is the go-118-fuzz-build /testing shim —
# argo's giant graph is never touched.
set -euo pipefail
...
TARGET="fuzz_progress"
STAGE="$SRC/_mayhem_harness/progress"
...
# ── Stage a standalone mini-module: progress.go (verbatim) + harness + KAT ─────
rm -rf "$STAGE"
mkdir -p "$STAGE/kat"
cp "$SRC/pkg/apis/workflow/v1alpha1/progress.go" "$STAGE/progress.go"
cp "$SRC/mayhem/harness_progress.go.src" "$STAGE/harness_progress.go"
cp "$SRC/mayhem/kat_main.go.src"          "$STAGE/kat/main.go"

(
  cd "$STAGE"
  go mod init github.com/argoproj/argo-workflows/v4/_mayhem_harness/progress
...
echo "=== go-118-fuzz-build $TARGET (func FuzzProgressParse) ==="
(
  cd "$STAGE"
  go-118-fuzz-build -func FuzzProgressParse -o "$SRC/mayhem-build/$TARGET.a" .
)
...
(
  cd "$STAGE"
  go build -o /mayhem/argo_progress_kat ./kat
)
file /mayhem/argo_progress_kat | grep -q 'dynamically linked' \
  || { echo "FATAL: /mayhem/argo_progress_kat is not dynamically linked — oracle would be reward-hackable"; exit 1; }
echo "built /mayhem/argo_progress_kat (dynamically linked)"
